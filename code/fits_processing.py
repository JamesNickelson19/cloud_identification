...
#############################################################################################################
    ...
    def save_image(self, filename):
        ...
        # test to make sure its actually ran properly
...
#############################################################################################################
    ...
    def generate_mask(self, mask_lwr, convolve=None ,gaussian_blur=None, median = False, filename="mask.FIT"):
        """
        ...
        if self.imgdata is not None:
            mask = np.median([img.data for img in self.imgdata], axis=0) # generating an image based off the median of the images from read_all_in_directory
...
#############################################################################################################
    ...
    def save_subregions(self, subregions = None):
        ...
        for subi in range(len(subregions)):
            ...
            plt.savefig(f"subregion_{subi}.png")
...
#############################################################################################################
    ...
    def exctract_features(self, subregions, mask = None):
        """
        ...
        for i, sub in enumerate(subregions):
            ...
            #print(sum((sub[src['y'].astype(int), src['x'].astype(int)]).astype(int)) / np.sum(sub.astype(int))) # gets number of sources divided by total sources in region. matches expected output
...
#############################################################################################################
    ...
    def write_model(self, filename = "lgb_model.pickle"):
...
#############################################################################################################
    ...
    def read_model(self, filename = "lgb_model.pickle"):
...
#############################################################################################################
    ...
    def train_model(self, parameters = {'max_depth' : 5, 'lambda' : 100,
        ...
        # also note that I've set the device to gpu, as this was an option for the xgboost model. for lgbm this has to be changed
        # when downloading
        xgb = XGBClassifier(objective = 'multi:softmax', num_class = 3, device = 'cuda', n_estimators = 500, random_state = 42, **parameters) 
...
#############################################################################################################
    ...
    def write_model(self, filename = "xgb_model.pickle"):
        """Write trained model to file."""
        self.filename = filename
        dump(self.model, filename)
...
#############################################################################################################
    ...
    def read_model(self, filename = "xgb_model.pickle"):
        """Read trained model from file."""
        self.filename = filename
        self.model = load(filename)
...
#############################################################################################################
"""
The following sections are just my tests to make sure everything is actually working as intended before going on
to actually training the models on my data.
"""
...
#test_image = processedFIT("C:/Users/james/OneDrive/Uni bits and bobs/Third Year/Final project/My code/AllSkyImage006286253.FIT")

#test_image.read_single_fit()

# these two functions should be commented out after generating the mask
#test_image.crop()
#test_image.save_image(filename="crop_test.FIT")

# mask generation parameters. generated using AllSkyImage007537912.FIT
#test_image.generate_mask(mask_lwr=5500, gaussian_blur=10)

# applying mask to test image. shouldn't be run at the same time as crop and generate_mask
#mask = test_image.crop_and_mask()

# generating subregions
#subregions = test_image.generate_subregions()
#testimage.save_subregions()

# testing the subregion overlay, and saving 
#overlay = test_image.create_overlay(overlaytype="subregions", regions=[1,2,3,4,5,6,7,8,9])
#test_image.write_image(filename="overlay_test.png", overlay=overlay)

# testing the remove outliers function
#run_algo = test_image.remove_outliers()

#features = test_image.exctract_features(subregions)
...
#lgbm_model.retrieve_training_data("C:/Users/james/OneDrive/Uni bits and bobs/Third Year/Final project/LightGBM/fulltrainingsample_features.dat")

#load_data = time.time() - start

#lgbm_model.train_model()
#lgbm_model.write_model(filename = "model_test.pickle")
...
#commented out as model has been trained
#xgb_model.retrieve_training_data("C:/Users/james/OneDrive/Uni bits and bobs/Third Year/Final project/LightGBM/fulltrainingsample_features.dat")

#load_data = time.time() - start

#xgb_model.train_model()
#xgb_model.write_model(filename = "xgb_model_test.pickle")
